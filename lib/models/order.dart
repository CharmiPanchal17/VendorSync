...
class StockItem {
  ...
  int get totalDelivered =>
      deliveryHistory.fold(0, (sum, record) => sum + record.quantity);
  ...
  int calculateTimeToThresholdAdjustment() {
    // If there is no delivery history or threshold, no adjustment
    if (deliveryHistory.isEmpty || thresholdLevel == 0) return 0;
    // Find all times when stock would have hit threshold after a delivery
    List<DateTime> thresholdHitDates = [];
    int runningStock = 0;
    // Sort deliveries by date ascending
    final sortedDeliveries = List<DeliveryRecord>.from(deliveryHistory)
      ..sort((a, b) => a.deliveryDate.compareTo(b.deliveryDate));
    for (final record in sortedDeliveries) {
      runningStock += record.quantity;
      if (runningStock <= thresholdLevel) {
        thresholdHitDates.add(record.deliveryDate);
        runningStock = 0; // Reset after hitting threshold
      }
    }
    if (thresholdHitDates.length < 2) return 0;
    // Calculate average days between threshold hits
    List<int> intervals = [];
    for (int i = 1; i < thresholdHitDates.length; i++) {
      intervals.add(
        thresholdHitDates[i].difference(thresholdHitDates[i - 1]).inDays.abs(),
      );
    }
    if (intervals.isEmpty) return 0;
    final avgInterval = intervals.reduce((a, b) => a + b) / intervals.length;
    // If avgInterval is low (hits threshold quickly), increase order quantity
    // If avgInterval is high (rarely hits threshold), no/little adjustment
    if (avgInterval <= 7) {
      // Hits threshold every week or less
      return (minimumStock * 1.0).round(); // Large boost
    } else if (avgInterval <= 14) {
      // Hits threshold every 2 weeks
      return (minimumStock * 0.7).round();
    } else if (avgInterval <= 30) {
      // Hits threshold every month
      return (minimumStock * 0.4).round();
    } else if (avgInterval <= 60) {
      // Hits threshold every 2 months
  ...
  int calculateBaseQuantity() {
    ...
    final recentDeliveries = deliveryHistory
        .where(
          (record) => record.deliveryDate.isAfter(
            DateTime.now().subtract(Duration(days: analysisDays)),
          ),
        )
        .toList();
    ...
    final daysSinceFirstDelivery = DateTime.now()
        .difference(recentDeliveries.first.deliveryDate)
        .inDays;
    final avgDailyUsage = daysSinceFirstDelivery > 0
  ...
  double _calculateDemandTrend(List<DeliveryRecord> recentDeliveries) {
    ...
    final earlyAvg =
        earlyDeliveries.fold(0, (sum, record) => sum + record.quantity) /
        earlyDeliveries.length;
    final recentAvg =
  ...
  int calculateSeasonalAdjustment() {
    ...
    final closestSeason = _getClosestSeason(
      daysUntilEaster,
      daysUntilIdd,
      daysUntilChristmas,
      daysUntilBackToSchool,
      daysUntilSuccessCards,
    );

    if (closestSeason == null) return 0; // No upcoming season

    // Check if this specific product is relevant to the upcoming season
    final isRelevantToSeason = _isRelevantToSeason(
  ...
  bool isSeasonalItem() {
    ...
    return seasonalKeywords.any(
      (keyword) => productNameLower.contains(keyword),
    );
  }
  ...
  bool _isRelevantToSeason(String seasonName) {
    ...
    switch (seasonName) {
      ...
      case 'BackToSchool':
        ...
        return backToSchoolKeywords.any(
          (keyword) => productNameLower.contains(keyword),
        );
      ...
      case 'SuccessCards':
        ...
        return successCardKeywords.any(
          (keyword) => productNameLower.contains(keyword),
        );
  ...
  Map<String, dynamic>? _getClosestSeason(
    int daysUntilEaster,
    int daysUntilIdd,
    int daysUntilChristmas,
    int daysUntilBackToSchool,
    int daysUntilSuccessCards,
  ) {
    ...
    upcomingSeasons.sort(
      (a, b) => (a['days'] as int).compareTo(b['days'] as int),
    );
    return {
  ...
  int calculateSeasonAdjustment(Map<String, dynamic> season) {
    ...
    final baseAdjustments = {
      ...
      'BackToSchool':
          0.8, // 80% increase for Back-to-School (higher due to longer season)
      'SuccessCards':
          1.2, // 120% increase for Success Cards (drastic increase as requested)
    };
    ...
    if (seasonName == 'BackToSchool') {
      ...
      if (daysUntilSeason <= 0) {
        ...
        return (minimumStock * baseAdjustment * 2.5)
            .round(); // 300% total increase
      } else if (daysUntilSeason <= 7) {
        // Very close to success cards season - very high adjustment
        return (minimumStock * baseAdjustment * 2.0)
            .round(); // 240% total increase
      } else if (daysUntilSeason <= 14) {
        // Close to success cards season - high adjustment
        return (minimumStock * baseAdjustment * 1.8)
            .round(); // 216% total increase
      } else if (daysUntilSeason <= 30) {
        // Approaching success cards season - moderate adjustment
        return (minimumStock * baseAdjustment * 1.5)
            .round(); // 180% total increase
      } else if (daysUntilSeason <= 60) {
        // Planning for success cards season - slight adjustment
        return (minimumStock * baseAdjustment * 1.2)
            .round(); // 144% total increase
      }
    } else {
  ...
  int calculateDemandAdjustment() {
    ...
    final firstHalf = recentDeliveries
        .take(recentDeliveries.length ~/ 2)
        .toList();
    final secondHalf = recentDeliveries
        .skip(recentDeliveries.length ~/ 2)
        .toList();

    final firstHalfAvg =
        firstHalf.fold(0, (sum, record) => sum + record.quantity) /
        firstHalf.length;
    final secondHalfAvg =
        secondHalf.fold(0, (sum, record) => sum + record.quantity) /
        secondHalf.length;

    final demandChange = secondHalfAvg - firstHalfAvg;
    final demandChangePercent = firstHalfAvg > 0
        ? demandChange / firstHalfAvg
        : 0.0;

    // Combine slope analysis with traditional demand change
    final slopeAdjustment = _calculateSlopeAdjustment(slope);
    final traditionalAdjustment = _calculateTraditionalAdjustment(
      demandChangePercent,
    );
