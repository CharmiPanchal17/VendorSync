...
class _StockManagementScreenState extends State<StockManagementScreen> {
  ...
  Future<void> _loadStockData() async {
    try {
      ...
      print(
        'DEBUG: Firestore stock_items found: \'${stockSnapshot.docs.length}\'',
      );
      for (final doc in stockSnapshot.docs) {
        final data = doc.data();
        print(
          'DEBUG: StockItem docId: ${doc.id}, vendorEmail: ${data['vendorEmail']?.toString() ?? 'NULL'}',
        );
      }
      ...
      if (stockSnapshot.docs.isNotEmpty) {
        ...
        final loadedStockItems = stockSnapshot.docs.map((doc) {
          final data = doc.data();
          print(
            'DEBUG:  [36m${data['productName']} thresholdLevel: ${data['thresholdLevel']}\u001b[0m',
          );
          return StockItem(
            id: doc.id,
        ...
        for (final item in stockItems) {
          print(
            '  Product: \'${item.productName}\', Stock: \'${item.currentStock}\', Vendor: \'${item.vendorEmail}\'',
          );
        }
      } else {
        setState(() {
  ...
  Future<void> _createStockFromOrders({bool initialSetup = false}) async {
    try {
      ...
      for (final entry in productGroups.entries) {
        ...
        for (final orderDoc in orders) {
          ...
          deliveryHistory.add(
            DeliveryRecord(
              ...
              quantity: quantity,
              supplierName:
                  data['supplierName'] as String? ?? 'Unknown Supplier',
              supplierEmail:
                  data['supplierEmail'] as String? ?? 'unknown@example.com',
        ...
        final currentStock = (totalDelivered * 0.7)
            .round(); // Assume 70% of delivered is still in stock
        final minimumStock = (totalDelivered * 0.1)
            .round(); // 10% of total delivered as minimum
        final averageUnitPrice = priceCount > 0
        ...
        realStockItems.add(
          StockItem(
            ...
            primarySupplier:
                (orders.first.data() as Map<String, dynamic>)['supplierName']
                    as String? ??
                'Unknown Supplier',
            primarySupplierEmail:
                (orders.first.data() as Map<String, dynamic>)['supplierEmail']
                    as String? ??
                'unknown@example.com',
  ...
  Future<void> _updateStockItem(int index, StockItem updatedStockItem) async {
    ...
    await FirebaseFirestore.instance.runTransaction((transaction) async {
      ...
      if (stockSnapshot.exists) {
        ...
        transaction.update(docRef, {
          ...
          'thresholdNotificationsEnabled':
              updatedStockItem.thresholdNotificationsEnabled,
    ...
    print(
      'DEBUG: Firestore transaction update for ${updatedStockItem.id} - new currentStock: $newCurrentStock',
    );

    final newStock = updatedStockItem.currentStock;
    final quantitySold = oldStock - newStock;
    print(
      'DEBUG: oldStock: $oldStock, newStock: $newStock, quantitySold: $quantitySold',
    );
    if (newStock < oldStock) {
      print(
        'DEBUG: Attempting to write sales record for ${updatedStockItem.productName}, qty: $quantitySold',
      );
      try {
        ...
        print(
          'DEBUG: Sales record written for ${updatedStockItem.productName}, qty: $quantitySold',
        );
      } catch (e) {
    ...
    if (!autoOrderPending &&
        newCurrentStock <= lowStockThreshold &&
        autoOrderQuantity > 0 &&
        supplierEmail != null &&
        supplierName != null) {
      // Create a new order
      final orderRef = await FirebaseFirestore.instance
          ...
          .add({
            ...
            'vendorEmail': updatedStockItem.vendorEmail,
            'status': 'Pending',
            'preferredDeliveryDate': DateTime.now().add(
      ...
      await inventory.reference.update({
      ...
      await NotificationService.createNotification(
        ...
        message:
            'An auto-order for ${updatedStockItem.productName} ($autoOrderQuantity units) has been created and is pending your confirmation.',
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              'Auto-order placed for ${updatedStockItem.productName} ($autoOrderQuantity units). Vendor notified for confirmation.',
            ),
          ...
          IconButton(
            ...
            onPressed: () async {
              ...
              if (mounted) {
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(
                    content: Text('Stock items synced from delivered orders!'),
                  ),
                );
              }
            },
  ...
  Widget _buildStockCard(
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: isAtThreshold ? Border.all(color: Colors.red, width: 2) : null,
        boxShadow: [
          BoxShadow(
            color: isAtThreshold
      ...
      child: ExpansionTile(
        ...
        title: Column(
          ...
          children: [
            ...
            if (stockItem.primarySupplierEmail != null &&
                stockItem.primarySupplierEmail!.isNotEmpty)
              Row(
                children: [
                  Icon(
                    Icons.email,
        ...
        trailing: Container(
          ...
          child: Icon(Icons.expand_more, color: maroon, size: 20),
        ),
        children: [_buildStockDetails(context, stockItem, isDark, index)],
      ),
    );
  }
          ...
          Row(
            children: [
              Container(
                ...
                child: Icon(Icons.analytics, color: statusColor, size: 20),
              ),
                  ...
                  children: [
                    ...
                    Container(
                      ...
                      width:
                          ...
                          0.6 *
                          stockItem.stockPercentage,
                      decoration: BoxDecoration(
                        gradient: LinearGradient(
                          colors: [
                            _getStockProgressColor(stockItem, isDark),
                            _getStockProgressColor(
                              stockItem,
                              isDark,
                            ).withOpacity(0.7),
                          ],
  ...
  Widget _buildSupplierInfo(StockItem stockItem, bool isDark) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        borderRadius: BorderRadius.circular(16),
        border: Border.all(color: maroon.withOpacity(0.2), width: 1),
      ),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Row(
            children: [
              Container(
                ...
                child: Icon(Icons.business, color: maroon, size: 20),
              ),
              const SizedBox(width: 12),
              Text(
                'Primary Supplier',
            ...
            child: Row(
              children: [
                Container(
                  ...
                  child: Icon(Icons.business, color: maroon, size: 24),
                ),
          ...
          if (stockItem.averageUnitPrice != null) ...[
            ...
            Container(
              ...
              child: Row(
                children: [
                  Icon(Icons.attach_money, color: maroon, size: 20),
                  const SizedBox(width: 8),
                  Text(
                    'Average Unit Price',
                    style: TextStyle(
  ...
  Widget _buildDeliveryRecord(DeliveryRecord record, bool isDark) {
    return Container(
      ...
      child: Row(
        children: [
          Icon(Icons.check_circle, color: maroon, size: 16),
          const SizedBox(width: 8),
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
          ...
          Row(
            children: [
              Container(
                ...
                child: Icon(Icons.settings, color: maroon, size: 20),
              ),
              const SizedBox(width: 12),
              Text(
                'Actions',
  ...
  void _navigateToAnalytics(StockItem stockItem) {
    Navigator.of(
      context,
    ).pushNamed('/vendor-product-analytics', arguments: stockItem.productName);
  }
                ...
                try {
                  ...
                  if (parentContext.mounted) {
                    ...
                    ScaffoldMessenger.of(parentContext).showSnackBar(
                      SnackBar(
                        content: Text(
                          '$qty units of ${stockItem.productName} have been sold.',
                        ),
                        backgroundColor: maroon,
  ...
  List<StockItem> _sortStockItems(List<StockItem> items) {
    ...
    for (int i = 0; i < items.length; i++) {
      ...
      print(
        '  ${i + 1}. ${item.productName} - Current: ${item.currentStock}, Min: ${item.minimumStock}, At Threshold: $isAtThreshold',
      );
    }
        ...
        actions: [
          ...
          ElevatedButton(
            ...
            onPressed: () async {
              ...
              final maximumStock =
                  int.tryParse(maximumStockController.text) ?? 0;
              ...
              final newStockItem = StockItem(
                ...
                primarySupplierEmail: supplierEmail.isNotEmpty
                    ? supplierEmail
                    : null,
