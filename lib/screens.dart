// This file will export all screen files for easy import.
// Example:
// export 'screens/splash_screen.dart'; 

export 'screens/splash_screen.dart';
export 'screens/welcome_screen.dart';
export 'screens/login_screen.dart';
export 'screens/register_screen.dart';
export 'screens/register_supplier_screen.dart';
export 'screens/reset_password_screen.dart';
export 'screens/role_selection_screen.dart';
export 'screens/offline_screen.dart';
// Vendor screens
export 'screens/vendor/dashboard_screen.dart';
export 'screens/vendor/create_order_screen.dart';
export 'screens/vendor/order_details_screen.dart';
export 'screens/vendor/notifications_screen.dart';
export 'screens/vendor/profile_screen.dart';
export 'screens/vendor/suppliers_list_screen.dart';
export 'screens/vendor/add_supplier_screen.dart';
export 'screens/vendor/invoice_list_screen.dart';
export 'screens/vendor/invoice_screen.dart';
// Supplier screens
export 'screens/supplier/dashboard_screen.dart';
export 'screens/supplier/order_details_screen.dart';
export 'screens/supplier/delivery_schedule_screen.dart';
export 'screens/supplier/notifications_screen.dart';
export 'screens/supplier/profile_screen.dart';
export 'screens/register_suppliers_screen.dart'; 